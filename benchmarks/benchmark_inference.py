...
def main():
    ...
    args.model = "huggyllama/llama-7b"
    # args.initial_peers="/ip4/27.0.0.1/tcp/31340/p2p/QmNTcejDSQenBj9sjLWViZw5vVYRXQDtKF81QkR3bPZjug"
    ...
    with model.transformer.h.inference_session(max_length=args.seq_len) as sess:
        for step in range(args.seq_len):
            ...
            if step >= args.warmup_steps:
                ...
                print('\t\tcurent step total cost time (sec): ', perf_counter() - start_time)
                speed = 1 / np.mean(step_times)
                logger.info(f"{process_idx=} {step=} {speed=:.2f}")

    result_pipe.send(speed)
    time_end_log = perf_counter()
    print('total time cost (sec): ', time_end_log-time_start_log)
